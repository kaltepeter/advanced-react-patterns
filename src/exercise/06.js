...
const useControlledSwitchWarning = (controlPropValue, controlPropName, componentName) => {
...
const useOnChangeReadOnly = (
  ...
  const isControlled = controlPropValue != null;
  React.useEffect(() => {
    warning(!(!hasOnChange && isControlled && !readOnly), `A \`${controlPropName}\` prop was provided to \`${componentName}\` without an \`${onChangeProp}\` handler. This will result in a read-only \`${controlPropName}\` value. If you want it to be mutable, use \`${initialValueProp}\`. Otherwise, set either \`${onChangeProp}\` or \`${readOnlyProp}\`.`,)
  }, [componentName,
    controlPropName,
    isControlled,
    hasOnChange,
...
function useToggle({
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // eslint-disable-next-line react-hooks/rules-of-hooks
    useOnChangeReadOnly(controlledOn, 'on',
      'useToggle',
      Boolean(onChange),
      readOnly,
      'readOnly',
